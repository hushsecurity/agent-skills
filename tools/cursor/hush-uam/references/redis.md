...
- `aiven` — an Aiven-managed Valkey service. Hush connects with an Aiven REST API token and mints the user via the Aiven API; it resolves the host/port itself, so no connection fields are supplied.
