...
- **For required fields with no sensible default** (GCP `project_id`, hostnames, account
  ARNs), don't fake a default. Offer `I'll provide it now` plus a contextual alternative;
  the user picks `Other` and types it. Don't lump these into a `defaults` shortcut.
...
- **Skip what you already know** from the prompt — but be conservative. `type` is genuinely
  ...
  ask scope when a policy is involved, presenting the trio as Recommended but listing the
  alternatives so the user can opt out.
- **Never ask which namespace the CRs go in** — it is always `hush-security`. The
  *workload's* namespace, the `k8s:ns` attestation value, is a different thing and you
  always ask for it. (Terraform has no CR namespace; it has `deployment_ids` instead,
...
7. **Add a post-generation note** covering anything version-gated or risky:
   ...
   - *Kubernetes:* if the manifest uses `remoteName`, the uniqueness requirement.
